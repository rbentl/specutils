...
def equivalent_width(wave, flux, vel=0.0, specRange=None, continuum=1.):
    
    '''
        Return the equivalent width of a spectrum within a certain spectral range
        
        Parameters
        ========
        wave - input array of wavelengths
        flux - flux values
        velocity -- in km/s (default: 0 km/s)
        specRange - the range in wavelength to compute the equivalent width (default: None)
        continuum - value from which to compute line depth (default: 1)
    '''
    
    if specRange is None:
        specRange = [np.min(wave), np.max(wave)]
    
    shift_flux = shiftSpec(wave, flux, vel)

    pts_idx = np.where((wave >= specRange[0]) & (wave <= specRange[1]))
    
    depth = continuum-shift_flux[pts_idx]
    
    return np.trapz(depth, x=wave[pts_idx])
